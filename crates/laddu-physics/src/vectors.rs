...
/// # Examples
/// ```rust
/// use laddu_physics::vectors::Vec3;
///
    ...
    fn try_from(value: Vec<f64>) -> Result<Self, Self::Error> {
        if value.len() != 3 {
            return Err(LadduPhysicsError::custom(
                "Attempted to convert Vec<f64> to Vec3 for Vec with len != 3",
            ));
        }
        Ok(Self {
            x: value[0],
...
impl Vec3 {
    ...
    pub fn unit(&self) -> LadduPhysicsResult<Vec3> {
        let mag = self.mag();
        if mag <= 0.0 {
            return Err(LadduPhysicsError::invalid_value(
                "vector magnitude",
...
/// # Examples
/// ```rust
/// use laddu_physics::vectors::{Vec3, Vec4};
///
...
impl TryFrom<Vec<f64>> for Vec4 {
    ...
    fn try_from(value: Vec<f64>) -> Result<Self, Self::Error> {
        if value.len() != 4 {
            return Err(LadduPhysicsError::custom(
                "Attempted to convert Vec<f64> to Vec4 for Vec with len != 4",
            ));
        }
        Ok(Self {
            x: value[0],
...
impl Vec4 {
    ...
    pub fn beta(&self) -> LadduPhysicsResult<Vec3> {
        let e = self.e();
        if e <= 0.0 {
            return Err(LadduPhysicsError::invalid_value(
...
mod tests {
    ...
    fn test_vec_vector_conversion() {
        ...
        let v_from_vec: Vec3 = vector3.try_into().unwrap();
        ...
        let v_from_vec: Vec4 = vector4.try_into().unwrap();
        assert_eq!(v_from_vec, v);
    }
