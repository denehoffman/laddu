...
static COMPUTE_AMPLITUDE_COUNTER: AtomicU64 = AtomicU64::new(0);
static AMPLITUDE_USE_SITE_COUNTER: AtomicU64 = AtomicU64::new(0);

#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, Serialize, Deserialize)]
struct ComputeAmplitudeId(u64);

#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, Serialize, Deserialize)]
struct AmplitudeUseSiteId(u64);

fn next_compute_amplitude_id() -> ComputeAmplitudeId {
    ComputeAmplitudeId(COMPUTE_AMPLITUDE_COUNTER.fetch_add(1, Ordering::Relaxed))
}

fn next_amplitude_use_site_id() -> AmplitudeUseSiteId {
    AmplitudeUseSiteId(AMPLITUDE_USE_SITE_COUNTER.fetch_add(1, Ordering::Relaxed))
}
...
use crate::{
    amplitude::{Amplitude, AmplitudeUseSite},
    data::Dataset,
...
pub struct ExpressionRegistry {
    ...
    amplitude_ids: Vec<ComputeAmplitudeId>,
    amplitude_use_sites: Vec<AmplitudeUseSite>,
    amplitude_use_site_ids: Vec<AmplitudeUseSiteId>,
    resources: Resources,
}
...
impl ExpressionRegistry {
    fn singleton(mut amplitude: Box<dyn Amplitude>) -> LadduResult<Self> {
        ...
        let compute_id = next_compute_amplitude_id();
        let use_site_id = next_amplitude_use_site_id();
