...
.. note:: ``laddu`` only requires the column naming convention described in :ref:`the API reference <laddu.data.Dataset>`. Parquet remains the recommended container (and works in both Rust and Python), but the reader can also ingest generic ROOT TTrees via ``oxyroot`` and, in Python, AmpTools-style ROOT tuples through the ``backend='amptools'`` option which uses ``uproot`` internally. (The AmpTools backend is currently exclusive to the Python bindings.) This avoids the need for any intermediate conversion scripts when working with those formats.
